...
class TestingProgressDisplay(object):
    def __init__(self, numTests, progressBar, verbose=False):
    ...
    def update(self, test: TestCaseResult):
        self.completed += 1
        ...
        print(
            "%s: %s (%d of %d)"
            % (test.msg, test.test_name, self.completed, self.numTests)
        )
...
def test_progress_display():
    ...
    display = TestingProgressDisplay(len(test_results), p, True)
